...
func TestTransferTx(t *testing.T) {
	...
	fmt.Println(">> before: ", updatedAccount1.Balance, updatedAccount1.Balance)
	require.Equal(t, account1.Balance-int64(n)*amount, updatedAccount1.Balance)
	require.Equal(t, account2.Balance+int64(n)*amount, updatedAccount2.Balance)
}

/* TestTransferTxDeadlock test reverse transfer
example: account1 transfer to account2, and
...
func TestTransferTxDeadlock(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		go func() {
			_, err := store.TransferTx(context.Background(), TransferTrxParams{
